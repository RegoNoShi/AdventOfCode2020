...
const solvePart1 = (input: string[]): number => {
  ...
  for (const passport of input) {
    const matches = [...passport.matchAll(/(\w+):(.+?)(?:\s|$)/g)];
...
const runAndVerifyPart1 = ({ input, expectedOutput }: TestCase) => {
  const output = solvePart1(input.split('\n\n'));
...
const fieldsChecks: Map<string, (val: string) => boolean> = new Map([
  ['byr', (byr: string) => +byr >= 1920 && +byr <= 2002],
  ['iyr', (iyr: string) => +iyr >= 2010 && +iyr <= 2020],
  ['eyr', (eyr: string) => +eyr >= 2020 && +eyr <= 2030],
  ['hcl', (hcl: string) => hcl.match(/^#[0-9a-f]{6}$/) != null],
  ['ecl', (ecl: string) => validEcls.has(ecl)],
  ['pid', (byr: string) => byr.match(/^\d{9}$/) != null],
  [
    'hgt',
    (hgt: string) => {
      const match = hgt.match(/^(\d+)(\w+)$/);
      if (match && match[2] == 'cm') {
        return +match[1] >= 150 && +match[1] <= 193;
      } else if (match && match[2] == 'in') {
...
const solvePart2 = (input: string[]): number => {
  ...
  for (const passport of input) {
    ...
    const fields = new Set(requiredFields);
    for (const match of matches) {
      const fn = fieldsChecks.get(match[1]);
      if (fn && fn(match[2])) {
        fields.delete(match[1]);
      }
    }
...
const runAndVerifyPart2 = ({ input, expectedOutput }: TestCase) => {
  const output = solvePart2(input.split('\n\n'));
  if (output == expectedOutput) {
    console.log(`✅ Input: ${input} -> Output: ${output}`);
  } else {
    console.log(`❌ Input: ${input} -> Expected output ${expectedOutput}, got ${output}`);
