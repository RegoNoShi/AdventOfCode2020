...
const solvePart1 = (input: number[]): number => {
  let jolts = [...input].sort((a, b) => a - b);
  jolts = [0, ...jolts, jolts[jolts.length - 1] + 3];
  let prevJolts = jolts[0];
  let oneJoltDiff = 0,
    threeJoltDiff = 0;
  for (let i = 1; i < jolts.length; i++) {
    if (jolts[i] - prevJolts === 1) {
      oneJoltDiff += 1;
    } else if (jolts[i] - prevJolts === 3) {
      threeJoltDiff += 1;
    }
    prevJolts = jolts[i];
  }
...
const testCasesPart1: TestCase[] = [
  { input: [16, 10, 15, 5, 1, 11, 7, 19, 6, 12, 4], expectedOutput: 35 },
  {
    input: [
      28,
      33,
...
const solvePart2 = (input: number[]): number => {
  let jolts = [...input].sort((a, b) => a - b);
  const oneJoltDiffGroups = [];
  let currentOneJoltDiffGroupSize = 0;
  const diffs: number[] = [];
  jolts = [0, ...jolts, jolts[jolts.length - 1] + 3];
  let prevJolts = jolts[0];
  for (let i = 1; i < jolts.length; i++) {
    diffs.push(jolts[i] - prevJolts);
    if (jolts[i] - prevJolts === 1) {
      currentOneJoltDiffGroupSize += 1;
    } else {
      if (currentOneJoltDiffGroupSize > 1) {
        oneJoltDiffGroups.push(currentOneJoltDiffGroupSize);
      }
      currentOneJoltDiffGroupSize = 0;
    }
    prevJolts = jolts[i];
  }
  let combinations = 1;
  for (const groupSize of oneJoltDiffGroups) {
    combinations *=
      1 + (groupSize - 1) + (groupSize >= 3 ? factorial(groupSize - 1) / factorial(groupSize - 3) / 2 : 0);
  }
  return combinations;
};

const testCasesPart2: TestCase[] = [
  { input: [16, 10, 15, 5, 1, 11, 7, 19, 6, 12, 4], expectedOutput: 8 },
  {
    input: [
      28,
      33,
...
measuringExecutionTime(() => {
  const expectedOutput = 6044831973376;
