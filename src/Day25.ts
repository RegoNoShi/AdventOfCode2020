...
const solvePart1 = (input: number[]): number => {
  let subjectNumber = 7;
  let cardLoopSize = 1;
  while (subjectNumber !== input[0]) {
    subjectNumber = (subjectNumber * 7) % 20201227;
    cardLoopSize += 1;
  }

  let result = input[1];
  for (let i = 1; i < cardLoopSize; i += 1) {
    result = (result * input[1]) % 20201227;
  }
...
const testCasesPart1: TestCasePart1[] = [{ input: [5764801, 17807724], expectedOutput: 14897079 }];
