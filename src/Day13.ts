...
const solvePart1 = (input: string[]): number => {
  const earliestDepartTime = +input[0];
  const busDepartTimes = input[1]
    .split(',')
    .filter((n) => n !== 'x')
    .map((n) => +n);
  ...
  let bestBus = 0;
  for (const busDepartTime of busDepartTimes) {
    const busEarliestArrivalTime = Math.ceil(earliestDepartTime / busDepartTime) * busDepartTime;
    const waitTime = busEarliestArrivalTime - earliestDepartTime;
    if (waitTime < minWait) {
      minWait = waitTime;
      bestBus = busDepartTime;
    }
  }
...
const testCasesPart1: TestCasePart1[] = [{ input: ['939', '7,13,x,x,59,x,31,19'], expectedOutput: 295 }];
...
const testCasesPart2: TestCasePart2[] = [
  { input: '7,13,x,x,59,x,31,19', expectedOutput: 1068781 },
  { input: '17,x,13,19', expectedOutput: 3417 },
  { input: '67,7,59,61', expectedOutput: 754018 },
  { input: '67,x,7,59,61', expectedOutput: 779210 },
  { input: '67,7,x,59,61', expectedOutput: 1261476 },
  { input: '1789,37,47,1889', expectedOutput: 1202161486 },
];
...
measuringExecutionTime(() => {
  const expectedOutput = 402251700208309;
  const output = solvePart2(challengeInput[1]);
  if (output == expectedOutput) {
    console.log(`✅ Challenge: ${output}`);
  } else {
    console.log(`❌ Challenge: Expected output ${expectedOutput}, got ${output}`);
