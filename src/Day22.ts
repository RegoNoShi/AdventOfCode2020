...
const decksFromInput = (input: string[]) => {
  const decks: number[][] = [];
  let row = 1;
  for (let p = 0; p < 2; p += 1) {
    decks.push([]);
    while (input[row] !== '' && row < input.length) {
      decks[p].push(+input[row]);
      row += 1;
    }
    row += 2;
  }
...
const playGame = (player1Deck: number[], player2Deck: number[]) => {
  while (player1Deck.length !== 0 && player2Deck.length !== 0) {
    if (player1Deck[0] > player2Deck[0]) {
      player1Deck.push(player1Deck.shift()!, player2Deck.shift()!);
    } else {
      player2Deck.push(player2Deck.shift()!, player1Deck.shift()!);
    }
  }
};

const solvePart1 = (input: string[]): number => {
  const decks = decksFromInput(input);
  playGame(decks[0], decks[1]);
  const winnerDeck = decks[0].length === 0 ? decks[1] : decks[0];
  return winnerDeck.reduce((res, num, i) => res + num * (winnerDeck.length - i), 0);
};
...
const testCasesPart1: TestCase[] = [
  { input: ['Player 1:', '9', '2', '6', '3', '1', '', 'Player 2:', '5', '8', '4', '7', '10'], expectedOutput: 306 },
];
...
const playRecursiveGame = (player1Deck: number[], player2Deck: number[]): number => {
  const playedDecks = new Set<string>();
  while (player1Deck.length !== 0 && player2Deck.length !== 0) {
    let decks = `${player1Deck.join(',')}-${player2Deck.join(',')}`;
    if (playedDecks.has(decks)) {
      return 0;
    } else {
      playedDecks.add(decks);
    }

    const c1 = player1Deck.shift()!;
    const c2 = player2Deck.shift()!;

    let winner = c1 > c2 ? 0 : 1;
    if (player1Deck.length >= c1 && player2Deck.length >= c2) {
      winner = playRecursiveGame(player1Deck.slice(0, c1), player2Deck.slice(0, c2));
    }

    if (winner === 0) {
      player1Deck.push(c1, c2);
    } else {
      player2Deck.push(c2, c1);
    }
  }

  return player1Deck.length === 0 ? 1 : 0;
};

const solvePart2 = (input: string[]): number => {
  const decks = decksFromInput(input);
  const winner = playRecursiveGame(decks[0], decks[1]);
  return decks[winner].reduce((res, num, i) => res + num * (decks[winner].length - i), 0);
};

const testCasesPart2: TestCase[] = [
  { input: ['Player 1:', '43', '19', '', 'Player 2:', '2', '29', '14'], expectedOutput: 105 },
  { input: ['Player 1:', '9', '2', '6', '3', '1', '', 'Player 2:', '5', '8', '4', '7', '10'], expectedOutput: 291 },
];
